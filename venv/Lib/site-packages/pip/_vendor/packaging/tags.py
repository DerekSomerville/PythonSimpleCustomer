...
glibc_version = collections.namedtuple("Version", ["major", "minor"])
...
def mac_platforms(version=None, arch=None):
    ...
    """
    Yields the platform tags for a mac_os system.

    The `version` parameter is a two-item tuple specifying the mac_os version to
    ...
    if version >= (11, 0):
        ...
        # mac_os version earlier than 11.0 when the x86_64 part of the binary supports
        # that version of mac_os.
        if arch == "x86_64":
...
def _manylinux_tags(linux, arch):
    ...
    too_old_glibc2 = glibc_version(2, 16)
    if arch in {"x86_64", "i686"}:
        # On x86/i686 also oldest glibc to be supported is (2, 5).
        too_old_glibc2 = glibc_version(2, 4)
    current_glibc = glibc_version(*_get_glibc_version())
    ...
    for glibc_major in range(current_glibc.major - 1, 1, -1):
        glibc_max_list.append(glibc_version(glibc_major, _LAST_GLIBC_MINOR[glibc_major]))
