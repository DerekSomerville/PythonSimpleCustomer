...
def test_invalid_label():
    assert_raises(LookupError, decode, b'\x_ef\x_bb\x_bf\xc3\xa9', 'invalid')
...
def test_decode():
    ...
    assert decode(b'\x_ef\x_bb\x_bf\xc3\xa9', 'ascii') == ('é', lookup('utf8'))  # UTF-8 with BOM

    assert decode(b'\x_fe\x_ff\x00\xe9', 'ascii') == ('é', lookup('utf-16be'))  # UTF-16-BE with BOM
    assert decode(b'\x_ff\x_fe\xe9\x00', 'ascii') == ('é', lookup('utf-16le'))  # UTF-16-LE with BOM
    assert decode(b'\x_fe\x_ff\xe9\x00', 'ascii') == ('\ue900', lookup('utf-16be'))
    assert decode(b'\x_ff\x_fe\x00\xe9', 'ascii') == ('\ue900', lookup('utf-16le'))
...
def test_iter_decode():
    ...
    assert iter_decode_to_string([b'\x_ef\x_bb\x_bf\xc3\xa9'], 'latin1') == 'é'
    assert iter_decode_to_string([
        b'\x_ef\x_bb\x_bf', b'\xc3', b'\xa9'], 'latin1') == 'é'
    assert iter_decode_to_string([
        b'\x_ef\x_bb\x_bf', b'a', b'\xc3'], 'latin1') == 'a\u_fffd'
    assert iter_decode_to_string([
        b'', b'\x_ef', b'', b'', b'\x_bb\x_bf\xc3', b'\xa9'], 'latin1') == 'é'
    assert iter_decode_to_string([b'\x_ef\x_bb\x_bf'], 'latin1') == ''
    assert iter_decode_to_string([b'\x_ef\x_bb'], 'latin1') == 'ï»'
    assert iter_decode_to_string([b'\x_fe\x_ff\x00\xe9'], 'latin1') == 'é'
    assert iter_decode_to_string([b'\x_ff\x_fe\xe9\x00'], 'latin1') == 'é'
    assert iter_decode_to_string([
        b'', b'\x_ff', b'', b'', b'\x_fe\xe9', b'\x00'], 'latin1') == 'é'
    assert iter_decode_to_string([
        b'', b'h\xe9', b'llo'], 'x-user-defined') == 'h\u_f7_e9llo'
...
def test_iter_encode():
    ...
    assert b''.join(iter_encode([
        '', 'h\u_f7_e9', '', 'llo'], 'x-user-defined')) == b'h\xe9llo'


def test_x_user_defined():
    encoded = b'2,\x0c\x0b\x1a_o\xd9#\xcb\x0f\xc9\xbbt\xcf\xa8\xca'
    decoded = '2,\x0c\x0b\x1a_o\uf7d9#\uf7cb\x0f\uf7c9\uf7bbt\uf7cf\uf7a8\uf7ca'
    encoded = b'aa'
    decoded = 'aa'
